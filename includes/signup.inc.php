...
if (isset($_POST['submit'])) {
	...
	if (empty($nama) ||empty($username) || empty($password)|| empty($confirmpwd)) {
		header("Location: signup_pns.php?=empty");
		exit();
			...
			if ($resulCheck > 0) {
				header("Location: ../signup_pns.php?signup=usertaken");
				exit();
			}else{
				if ($password != $confirmpwd) {
					header("Location: ../signup_pns.php?signup=password_doesnt_match");
					exit();
				}else{
					$hashedPwd = password_hash($password, PASSWORD_DEFAULT);
					$sql = "INSERT INTO admin_pns (nama, username, password) VALUES ('$nama', '$username', '$hashedPwd');";
