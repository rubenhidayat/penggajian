...
if (isset($_POST['submit'])) {
	...
	if (empty($username) || empty($password)) {
		header("Location: ../login_honor.php?login=empty");
		exit();
	} else {
		...
		if ($resultCheck < 1) {
			header("Location: ../login_honor.php?login=error1");
				exit();
		} else {
			if ($row = mysqli_fetch_assoc($result)) {
				...
				if ($hasedPwdCheck == false) {
					header("Location: ../login_honor.php?login=error2");
					exit();
					...
					header("Location: ../admin_honor.php");
					exit();
				}
			}
		}
